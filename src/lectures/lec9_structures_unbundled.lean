...
import ..prooflab
import .lec8_structures_bundled
...
/- The unbundled definition of preorder: this is the type of preorder structure on  a type `X`. Note that this structure depends on the parameter `X` in contrast with the unbundles version which does not have any parameters. -/
...
def endo_of_auto {A : Type} (α : auto A) : endo A := 
α.to_fun
