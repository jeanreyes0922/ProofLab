...
begin
 ...
 {
  intro abz,
  have h₁, from eq_zero_or_eq_zero_of_mul_eq_zero abz,
  assumption,
 },
 {
  intro azbz,
...
lemma proof_by_contrapositive_alt_alt 
  (P Q : Prop) : (P → Q) → (¬Q → ¬P) := 
begin 
  intro hpq, 
  intro hnq,
...
-- Tactic __suffices__:
lemma proof_by_contrapositive_alt_alt_alt
(P Q : Prop) : (P → Q) → (¬Q → ¬P) := 
begin
  intro hpq, 
  intro hnq, 
